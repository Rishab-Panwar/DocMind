...
SUPPORTED_EXTENSIONS = {".pdf", ".docx", ".txt", ".pptx", ".md", ".csv", ".xlsx", ".xls", ".db", ".sqlite", ".sqlite3", ".json"}
...
def load_documents(paths: Iterable[Path]) -> List[Document]:
    ...
    try:
        for p in paths:
            ...
            try:
                if ext == ".pdf":
                    docs.extend(_read_pdf_smart(p))
                elif ext == ".json":
                    docs.append(_doc_from_text(_read_json(p), p, {"file_type": ext}))
                elif ext == ".docx":
...
def _read_json(p: Path) -> str:
    """Pretty-print JSON so its structure is readable and chunkable."""
    import json
...
def _tesseract_available() -> bool:
    """True only if pytesseract AND the Tesseract binary are installed."""
    try:
        import pytesseract
...
def _ocr_page(page) -> str:
    """OCR a single PyMuPDF page by rendering it to an image."""
    import io
    import pytesseract
...
_PDF_OCR_MIN_CHARS = 20


def _read_pdf_smart(p: Path) -> List[Document]:
    """Load a PDF page-by-page. Pages with little/no extractable text (scanned
    or image-only) fall back to OCR via PyMuPDF render + Tesseract. If Tesseract
    isn't installed, those pages stay empty and a warning is logged — the rest
    of the PDF still indexes normally."""
    import fitz  # PyMuPDF
    try:
        pdf = fitz.open(str(p))
    except Exception as e:
        # If PyMuPDF can't open it, fall back to the basic text loader.
        log.warning("PyMuPDF open failed; using PyPDFLoader", path=str(p), error=str(e))
    ...
    for i, page in enumerate(pdf):
        ...
        if len(text.strip()) < _PDF_OCR_MIN_CHARS:
            scanned_pages += 1
            if ocr_available:
                try:
                    ...
                    log.warning("OCR failed on page", page=i, path=str(p), error=str(e))
        docs.append(
            Document(page_content=text, metadata={"source": str(p.resolve()), "page": i, "file_type": ".pdf"})
        )
    pdf.close()

    if scanned_pages and not ocr_available:
        log.warning(
            "PDF has scanned/image pages but Tesseract OCR is not installed — "
            "those pages were left empty",
            path=str(p), scanned_pages=scanned_pages,
        )
    elif ocr_pages:
        log.info("PDF OCR applied", path=str(p), ocr_pages=ocr_pages)
    return docs
