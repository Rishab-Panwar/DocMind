...
def _parse_history(raw: Optional[str]) -> List[Dict[str, str]]:
    """Parse the chat_history form field (JSON list of {role, content})."""
    if not raw:
        return []
    try:
        data = json.loads(raw)
    ...
    return out[-12:]  # keep the last ~6 turns
...
async def chat_query(
    ...
    try:
        ...
        if multimodal:
            ...
            response = rag.invoke(question, chat_history=_parse_history(chat_history))
...
async def agent_query(
    ...
    try:
        ...
        answer = rag.invoke(question, chat_history=_parse_history(chat_history))
